...
// Version: 0.8 Speculoos 2018.03.08
    ...
    for(i=0;i<numEvents && numEventsShown<14;i++) {
        ...
        if(showAll || llList2Integer(timeParsed, 2) > notBefore) {
            ...
            if(llList2Integer(timeParsed, 2) < currentTime - 3600) {
                commandList = osSetPenColor(commandList, "ff00ff00");
            }
            else if(llList2Integer(timeParsed, 2) < currentTime) {
                commandList = osSetPenColor(commandList, "ff33ff33");
            }
            else if(llList2Integer(timeParsed, 2) < currentTime + 4 * 3600) {
                commandList = osSetPenColor(commandList, "cc00ff00");
            }
            else if(llList2Integer(timeParsed, 2) < currentTime + 24 * 3600) {
                commandList = osSetPenColor(commandList, "9900ff00");
            }
            else {
                commandList = osSetPenColor(commandList, "6600ff00");
            }
